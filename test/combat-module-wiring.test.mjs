...
test("Foundry loads free Helm, salvo, condition consequences, and salvo UI in dependency order", async () => {
  const manifest = JSON.parse(await readFile(modulePath, "utf8"));
  const modules = manifest.esmodules ?? [];
  const engagement = modules.indexOf("src/foundry/combat-engagement-api.js");
  const stationActions = modules.indexOf("src/foundry/combat-station-actions-api.js");
  ...
  const conditions = modules.indexOf("src/foundry/combat-condition-consequences.js");
  ...
  assert.ok(engagement > stationActions, "engagement runtime must load after station-action API registration");
  ...
  assert.ok(conditions > salvo, "condition consequences should observe final attack events");
