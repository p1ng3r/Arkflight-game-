...
Hooks.once("init", () => {
  ...
  game.arkflight = {
    ...
    async openEvent(eventId = "glassback-cinderwake", options = {}) {
      ...
      await controller.openEvent(eventId, options);
      renderBoard();
      return controller.state;
    },
    async endEvent() {
      if (!controller) throw new Error("Arkflight is not ready yet.");
      await controller.endEvent();
      if (board?.rendered) await board.close();
      if (rewardSummary?.rendered) await rewardSummary.close();
      return null;
    },
