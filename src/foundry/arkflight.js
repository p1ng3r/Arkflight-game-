...
function ensureGMOperations() { if (!game.user.isGM) return null; if (!gmOperations) gmOperations = new ArkflightGMOperations(); return gmOperations; }
...
function openGMOperations(options = {}) { if (!game.user.isGM) return null; return ensureGMOperations()?.open(options) ?? null; }
function showActiveEventChoice() {
  const DialogV2 = foundry.applications.api.DialogV2;
  return new DialogV2({ window: { title: "Arkflight Operations" }, content: "<p>An Arkflight event is already active. Resume the Event Board or open GM Operations.</p>", buttons: [
    { action: "resume", label: "Resume Event", icon: "fa-solid fa-compass", default: true, callback: () => renderBoard() },
    { action: "operations", label: "Open GM Operations", icon: "fa-solid fa-screwdriver-wrench", callback: () => openGMOperations({ section: "operations" }) }
  ] }).render({ force: true });
}
function baseStationOptions() { return Object.fromEntries(Object.entries(BASE_MASTERY).map(([stationId, masteries]) => [stationId, { masteries: [...masteries], signatures: [], componentAbilities: [] }])); }
function announceStateRewards(state) {
  ...
  if (state.phase === "round-result" && state.consequenceApplied) {
    ...
    if (key !== lastRoundRewardKey) { lastRoundRewardKey = key; const before = Number(state.roundMomentumBefore ?? state.encounter?.momentum ?? 0); const award = Number(state.roundMomentumAward ?? state.roundResult?.momentumDelta ?? 0); const after = Number(state.roundMomentumAfter ?? state.encounter?.momentum ?? 0); const tacticNames = (state.roundRewards?.awardedEdgeCards ?? []).map((id) => getCrewEdgeCard(id)?.name).filter(Boolean); const tacticText = tacticNames.length ? ` Crew Tactic earned: ${tacticNames.join(", ")}.` : ""; ui.notifications?.info(`Arkflight Momentum: ${before} ${award >= 0 ? "+" : ""}${award} → ${after}.${tacticText}`); }
  }
...
Hooks.once("init", () => {
  ...
  installMasteryTacticsUI(); installPlayerSetupClaims(); installPlayerResolutionUI(); installMasteryOpportunityUI(); installOpeningScreenUI();
  game.arkflight = {
    ...
    openGMOperations,
    async openEvent(eventId = "glassback-cinderwake", options = {}) { if (!controller) throw new Error("Arkflight is not ready yet."); await controller.openEvent(eventId, options); renderBoard(); return controller.state; },
    async endEvent() { if (!controller) throw new Error("Arkflight is not ready yet."); await controller.endEvent(); if (board?.rendered) await board.close(); if (rewardSummary?.rendered) await rewardSummary.close(); return null; },
    setStationOptions(stationId, options = {}) { const base = BASE_MASTERY[stationId] ?? []; this.stationOptions[stationId] = { masteries: [...(options.masteries ?? base)], signatures: [], componentAbilities: [...(options.componentAbilities ?? [])] }; if (board?.rendered) board.render({ force: true }); }
  };
  game.arkflight.ships = createShipService();
});
Hooks.once("ready", () => {
  controller = new PlanningController({ onStateChange: (state) => { const eventActive = Boolean(state?.eventId); if (eventActive !== lastEventActive) { lastEventActive = eventActive; ui.controls?.render(); } if (board?.rendered) renderBoard(); if (gmOperations?.rendered) gmOperations.render({ force: true }); announceStateRewards(state); } });
  controller.activateSockets(); lastEventActive = Boolean(controller.state?.eventId);
});
Hooks.on("getSceneControlButtons", (controls) => {
  if (!controls.tokens?.tools) return;
  const eventActive = Boolean(controller?.state?.eventId);
  controls.tokens.tools.arkflightEvent = { name: "arkflightEvent", title: game.user.isGM ? "Arkflight GM Operations" : "Arkflight Event Board", icon: "fa-solid fa-compass", order: Object.keys(controls.tokens.tools).length, button: true, visible: game.user.isGM || eventActive, onChange: async () => {
    if (!controller) return; const active = Boolean(controller.state?.eventId);
    if (!game.user.isGM) { if (active) renderBoard(); return; }
    if (active) { showActiveEventChoice(); return; }
    openGMOperations();
  } };
});