...
function escapeShipPickerText(value) {
  const raw = String(value ?? "");
  return foundry?.utils?.escapeHTML ? foundry.utils.escapeHTML(raw) : raw.replace(/[&<>"']/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#039;" })[char]);
}

async function chooseVoyageShip({ title = "Choose Arkflight Vessel", confirmLabel = "Use Vessel" } = {}) {
  if (!game.user?.isGM) throw new Error("Only the GM may choose the Arkflight vessel for an Event Manager launch.");
  const ships = commissionedShips().sort((a, b) => String(a.name ?? "").localeCompare(String(b.name ?? "")));
  ...
  const options = ships.map((actor) => `<option value="${escapeShipPickerText(actor.uuid)}">${escapeShipPickerText(actor.name || "Unnamed Vessel")}</option>`).join("");
  const result = await DialogV2.input({
    window: { title },
    content: `
      <div class="standard-form arkflight-vessel-picker">
        <p>Choose the commissioned Arkflight vessel this launch will use.</p>
        <div class="form-group">
          <label>Vessel</label>
          <div class="form-fields">
            <select name="shipUuid" autofocus>${options}</select>
          </div>
        </div>
      </div>
    `,
  ...
  const uuid = String(result.shipUuid ?? result.get?.("shipUuid") ?? "");
  return ships.find((actor) => actor.uuid === uuid) ?? null;
}

async function resolveVoyageShip(reference = null, pickerOptions = {}) {
  if (!reference) return chooseVoyageShip(pickerOptions);
...
async function bindVoyageShip(reference = null, pickerOptions = {}) {
  const actor = await resolveVoyageShip(reference, pickerOptions);
  if (!actor) return null;
  await game.settings.set(MODULE_ID, ACTIVE_SHIP_SETTING, actor.uuid);
  if (game.arkflight) game.arkflight.stationOptions = stationOptionsForShip(actor);
  return actor;
}
...
async function bindExistingEventShipIfNeeded() {
  ...
  if (commissionedShips().length) ui.notifications?.warn("The active Arkflight Event has no vessel bound. Reopen it from Event Manager and choose the vessel.");
  return null;
}
  ...
  game.arkflight = {
    ...
    async openEvent(eventId = "glassback-cinderwake", shipReference = null) {
      ...
      const shipActor = await bindVoyageShip(shipReference, {
        title: `Choose Vessel — ${ARKFLIGHT_EVENTS[eventId]?.title ?? "Arkflight Event"}`,
        confirmLabel: "Launch Event"
      });
