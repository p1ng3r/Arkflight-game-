...
function pricedItem(item, mode, operation) {
  ...
  else if (operation === "install") partsCost = quote.installation[mode]?.aetherScrap ?? quote.installation.crew.aetherScrap;
  return {
    ...item,
    data: {
      ...item.data,
...
function economicCatalogs(mode, operation) {
  return {
    ...SHIP_CATALOGS,
    shipMods: Object.fromEntries(Object.entries(SHIP_CATALOGS.shipMods ?? {}).map(([id, item]) => [id, pricedItem(item, mode, operation)])),
    arkengineMods: Object.fromEntries(Object.entries(SHIP_CATALOGS.arkengineMods ?? {}).map(([id, item]) => [id, pricedItem(item, mode, operation)]))
  };
}
...
function repairInstallSocketBeforeStart(ship, jobId) {
  ...
  const current = validateRefitSocketAssignment(ship, SHIP_CATALOGS, { family: job.componentFamily, componentId: job.componentId, socketIndices: job.socketIndices });
...
Hooks.once("init", () => {
  ...
  game.arkflight.refit = Object.freeze({
    ...
    async beginInstallDraft(actor, draft, options = {}) {
      ...
      const result = queueInstallDraft(shipPayload(target), draft, economicCatalogs(mode, "install"), { ...options, method: underlyingMethod(options) });
      return persistResult(target, tagService(result, mode));
    },
    ...
    async queueRemove(actor, family, componentId, options = {}) { const target = targetForRefit(actor); const mode = serviceMode(options); const result = queueRemoveJob(shipPayload(target), family, componentId, SHIP_CATALOGS, { ...options, method: underlyingMethod(options) }); return persistResult(target, tagService(result, mode)); },
