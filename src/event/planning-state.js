...
export function createPlanningState({ eventId, roundId, roundIndex = 0, now = Date.now(), assignments = null, masterySelections = null, availableMasteries = null, crewEdgeHand = [] }) {
  return { version: 5, eventId, roundId, roundIndex, phase: "opening", createdAt: now, planningStartedAt: null, planningEndsAt: null, lockedAt: null, setupLocked: false, order: [...STATIONS], assignments: assignments ? structuredClone(assignments) : emptyAssignments(), availableMasteries: availableMasteries ? structuredClone(availableMasteries) : emptyMasteryAvailability(), masterySelections: masterySelections ? structuredClone(masterySelections) : emptyMasterySelections(), masteryUses: {}, reopenedStations: {}, crewEdgeHand: [...crewEdgeHand], pendingShipEffects: [], selections: Object.fromEntries(STATIONS.map((station) => [station, emptySelection()])) };
}
...
export function eventSetupReady(state) {
  ...
  return STATIONS.every((station) => {
    const masteryId = state.masterySelections?.[station];
    if (!masteryId) return false;
    const allowed = state.availableMasteries?.[station] ?? [];
    return !allowed.length || allowed.includes(masteryId);
  });
}
