...
function emptyMasteryAvailability() { return Object.fromEntries(STATIONS.map((station) => [station, []])); }

export function createPlanningState({ eventId, roundId, roundIndex = 0, now = Date.now(), assignments = null, masterySelections = null, availableMasteries = null, crewEdgeHand = [] }) {
  return {
    version: 5,
    ...
    availableMasteries: availableMasteries ? structuredClone(availableMasteries) : emptyMasteryAvailability(),
...
export function assignActor(state, station, actorId) {
  ...
  if (actorId) { const duplicate = STATIONS.find((other) => other !== station && state.assignments?.[other]?.actorId === actorId); if (duplicate) throw new Error("That PF2e character is already assigned to another Arkflight station for this Event."); }
...
export function selectMastery(state, station, masteryId) {
  ...
  if (masteryId && !allowed.includes(masteryId)) throw new Error("That Mastery is not available from the bound Arkflight vessel.");
...
export function restartEvent(state, { roundId, preserveAssignments = true, preserveCrewEdgeHand = true, preserveMastery = true, now = Date.now() } = {}) {
  ...
  return createPlanningState({ eventId: state.eventId, roundId: roundId ?? state.roundId, roundIndex: 0, now, assignments: preserveAssignments ? (state.assignments ?? emptyAssignments()) : null, masterySelections: preserveMastery ? (state.masterySelections ?? emptyMasterySelections()) : null, availableMasteries: state.availableMasteries ?? emptyMasteryAvailability(), crewEdgeHand: preserveCrewEdgeHand ? (state.crewEdgeHand ?? []) : [] });
}
