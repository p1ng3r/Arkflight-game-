...
export function applyCrewTactic(state, tacticId, options = {}) {
  const tactic = getCrewEdgeCard(tacticId); if (!tactic) throw new Error(`Unknown Crew Tactic: ${tacticId}`); if (!(state?.crewEdgeHand ?? []).includes(tacticId)) throw new Error(`${tactic.name} is not currently Ready.`); if (["opening", "event-complete"].includes(state.phase) && tacticId !== "crew-instinct") throw new Error("That Crew Tactic cannot be spent in this phase.");
  ...
  switch (tacticId) {
    ...
    case "crew-instinct": { if (!["opening", "round-opening"].includes(state.phase)) throw new Error("Crew Instinct is used before planning begins."); const desired = Array.isArray(options.order) ? options.order : []; if (desired.length !== state.order.length || new Set(desired).size !== state.order.length || desired.some((id) => !state.order.includes(id))) throw new Error("Crew Instinct requires a complete valid station order."); next = { ...next, order: [...desired] }; break; }
