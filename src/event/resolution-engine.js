...
export function selectedResolution(event, state, stationId) {
  ...
  return { stationId, selection, action, skill, riskBid, riskBenefit, momentumBonus, checkBonus: adjustments.bonus, checkBonusSources: adjustments.bonusSources, dcAdjustment: adjustments.dc, dcAdjustmentSources: adjustments.dcSources, degreeLift: adjustments.degreeLift, degreeLiftSources: adjustments.degreeLiftSources, authoredRiskIncrease, riskIncrease, riskReducedByMastery, preAdjustmentDc, finalDc: Math.max(0, preAdjustmentDc + Number(adjustments.dc ?? 0)) };
}

export async function rollSelectedStation({ event, state, actor }) {
  const stationId = activeStationId(state); if (!stationId) throw new Error("No station is waiting to resolve."); const chosen = selectedResolution(event, state, stationId); if (!chosen) throw new Error(`The ${stationId} selection is incomplete or invalid.`); if (!actor) throw new Error("The assigned PF2e actor is unavailable.");
  const modifiers = []; if (chosen.momentumBonus) modifiers.push({ slug: "arkflight-momentum", label: "Arkflight Momentum", modifier: chosen.momentumBonus, type: "untyped", source: `Crew Momentum ${chosen.momentumBonus}/3` }); if (chosen.checkBonus) modifiers.push({ slug: "arkflight-crew-advantage", label: "Arkflight Crew Advantage", modifier: chosen.checkBonus, type: "untyped", source: chosen.checkBonusSources?.join("; ") || "Arkflight Heroic / Risk Benefit" });
  const roll = await rollPf2eStatistic({ actor, statisticSlug: chosen.skill.skill, dc: chosen.finalDc, modifiers, label: `Arkflight — ${chosen.action.name}`, options: ["arkflight:event", `arkflight:station:${stationId}`, `arkflight:action:${chosen.action.id}`, `arkflight:momentum:${chosen.momentumBonus}`, ...(chosen.riskBid ? [`arkflight:risk:${chosen.riskBid.tier}`] : [])] });
  return { stationId, chosen, roll };
}

export function applyStationRollResult({ event, state, actor, roll }) {
  const stationId = activeStationId(state); if (!stationId) throw new Error("No station is waiting to resolve."); const chosen = selectedResolution(event, state, stationId); if (!chosen) throw new Error(`The ${stationId} selection is incomplete or invalid.`); if (!actor) throw new Error("The assigned PF2e actor is unavailable."); if (!roll || !Number.isFinite(Number(roll.total)) || !roll.outcome) throw new Error("Arkflight received an invalid PF2e roll result.");
  ...
  if (postCheckShipEffects.length) {
    nextState = { ...nextState, pendingShipEffects: [...(nextState.pendingShipEffects ?? []), ...postCheckShipEffects.map((effect) => ({ ...effect }))], encounter: { ...nextState.encounter, notes: [...(nextState.encounter?.notes ?? []), ...postCheckShipEffects.map((effect) => `${effect.source ?? "Mastery"}: ${effect.value >= 0 ? "+" : ""}${effect.value} Strain threatening ${effect.area ?? "the ship"} after ${stationId} resolved.`)] } };
  }
...
export async function resolveActiveStation({ event, state, actor }) { const rolled = await rollSelectedStation({ event, state, actor }); return applyStationRollResult({ event, state, actor, roll: rolled.roll }); }
