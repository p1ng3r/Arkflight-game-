...
function addPressure(encounter, system, value) {
  encounter.pressure[system] = Math.max(0, Number(encounter.pressure[system] ?? 0) + Number(value ?? 0));
}

function addHazard(encounter, hazardId) {
  if (hazardId && !encounter.hazards.includes(hazardId)) encounter.hazards.push(hazardId);
}
...
function targetStation(state, riskBid) {
  ...
  const unresolved = state.order.filter((id) => !state.results?.[id]);
  return unresolved[0] ?? null;
}

export function encounterFromEvent(event) {
  return cloneEncounter(event?.startingState ?? {});
}
...
export function applyEarnedRiskBenefit(state, chosen, degreeKey) {
  if (!chosen?.riskBid || !chosen?.riskBenefit) return state;
  if (!(degreeKey === "success" || degreeKey === "criticalSuccess")) return state;
  ...
  const bonusTarget = (value) => {
    if (target) encounter.checkBonuses[target] = Number(encounter.checkBonuses[target] ?? 0) + value;
  };
  const dcTarget = (value) => {
    if (target) encounter.dcAdjustments[target] = Number(encounter.dcAdjustments[target] ?? 0) + value;
  };

  switch (id) {
    case "aid-next-1":
      bonusTarget(critical ? 2 : 1);
      break;
    case "dc-next-1":
      dcTarget(critical ? -2 : -1);
      break;
    case "crew-surge":
      for (const stationId of unresolved) encounter.checkBonuses[stationId] = Number(encounter.checkBonuses[stationId] ?? 0) + (critical ? 2 : 1);
      break;
    case "order-followup":
      if (target) {
      ...
      break;
    case "arkengine-vent":
      addPressure(encounter, "arkengine", critical ? -2 : -1);
      break;
    case "lifeveil-steady":
      addPressure(encounter, "lifeveil", critical ? -2 : -1);
      break;
    case "arkengine-overdrive":
      bonusTarget(critical ? 3 : 2);
      break;
    case "arkengine-master":
      addPressure(encounter, "arkengine", -2);
      if (critical) encounter.momentum = clampMomentum(encounter.momentum + 1);
      break;
    case "degree-lift":
      if (target) encounter.degreeLifts[target] = Math.max(Number(encounter.degreeLifts[target] ?? 0), 1);
      break;
    case "order-swap":
      if (target) {
        const activeIndex = Number(state.activeOrderIndex ?? 0);
    ...
    case "hazard-reveal-1":
      encounter.notes.push(critical ? "A hidden danger was revealed with an exploitable opening." : "A hidden danger was revealed before it struck.");
      break;
    default:
      encounter.notes.push(`${chosen.riskBenefit.name} was earned; its authored payoff remains available for GM adjudication.`);
      break;
  }
...
export function cinematicRoundNarrative({ round, bandId, results, consequenceNarrative }) {
  ...
  const opening = {
    extraordinary: `For a few fierce breaths, the crew owns ${round.title.toLowerCase()}.`,
    "strong-success": `The ship drives through ${round.title.toLowerCase()} with the crew moving as one.`,
    "mixed-success": `The ship claws its way through ${round.title.toLowerCase()}, but the passage is anything but clean.`,
    failure: `The crossing through ${round.title.toLowerCase()} begins to come apart around the crew.`,
    disaster: `Everything in ${round.title.toLowerCase()} turns violent at once.`
  }[bandId] ?? `The crew fights through ${round.title.toLowerCase()}.`;
  const middle = strong && weak
    ? `${stationPhrase(strong)}, buying precious room even as ${stationPhrase(weak)}.`
    : strong
      ? `${stationPhrase(strong)}, giving the ship the opening it needs.`
      : weak
        ? `${stationPhrase(weak)}, and the rest of the crew is forced to compensate.`
        : `Every station strains against the same narrowing window.`;
...
export function finalizeRound(event, state) {
  ...
  const roundNarrative = cinematicRoundNarrative({
    round,
    bandId: state.roundResult.bandId,
...
export function advanceToNextRound(event, state, now = Date.now()) {
  ...
  return {
    ...
    selections: Object.fromEntries(Object.entries(state.selections).map(([stationId, selection]) => [
      stationId,
      { ...selection, actionId: null, skillId: null, riskTier: null, componentAbilityId: null }
    ]))
  };
}
