...
const HandlebarsActorSheet = HandlebarsApplicationMixin(ActorSheetV2);
...
export class ArkflightShipSheet extends HandlebarsActorSheet {
  constructor(options = {}) {
    super(options);
  ...
  get title() { return `${this.actor?.name ?? "Arkflight Vessel"} — Arkflight Vessel`; }

  async _prepareContext(options = {}) {
    const context = await super._prepareContext(options);
    const actor = this.actor;
    const ship = shipFlag(actor) ?? createShip({ identity: { name: actor.name || "Unnamed Vessel" } });
    ...
    return {
      ...context,
      actor,
      cssClass: "arkflight arkflight-ship-sheet",
      arkflight: {
        marked: isArkflightShip(actor), actorUuid: actor.uuid, actorImg: actor.img,
        isGM: game.user.isGM, canOperate: actor.isOwner, canManageRefit: Boolean(game.user.isGM || actor.isOwner),
  ...
  async _onRender(context, options) {
    ...
    for (const button of html.querySelectorAll("[data-tab]")) button.addEventListener("click", (event) => {
    ...
    for (const input of html.querySelectorAll("[data-resource]")) input.addEventListener("change", async (event) => {
      ...
      const max = statValue(dynamicMax);
    ...
    for (const button of html.querySelectorAll("[data-compendium-pack]")) button.addEventListener("click", (event) => {
      event.preventDefault();
      const packId = event.currentTarget.dataset.compendiumPack;
      const pack = game.packs?.get(packId);
      if (!pack) return ui.notifications?.warn(`Arkflight Compendium pack is not available yet: ${packId}`);
      pack.render(true);
    });
    ...
    for (const button of html.querySelectorAll("[data-refit-remove]")) button.addEventListener("click", async (event) => {
      event.preventDefault(); if (!(game.user.isGM || this.actor.isOwner)) return;
      const family = event.currentTarget.dataset.family; const id = event.currentTarget.dataset.id;
      const layout = installedSocketLayout(shipFlag(this.actor), SHIP_CATALOGS, family);
      const placement = layout.placements.find((entry) => entry.componentId === id);
      if (!placement) return ui.notifications?.warn("That fitting is not installed.");
      try {
        const queued = await game.arkflight?.refit?.queueRemove?.(this.actor, family, id, { method: "crew", socketIndices: [...placement.socketIndices], sourceInstallJobId: placement.sourceJobId ?? "" });
        await startQueued(this.actor, queued, `Remove ${catalogForFamily(family)?.[id]?.name ?? id}`);
    ...
    for (const button of html.querySelectorAll("[data-refit-start]")) button.addEventListener("click", async (event) => {
      event.preventDefault(); if (!(game.user.isGM || this.actor.isOwner)) return;
      try {
        const result = await game.arkflight?.refit?.startWork?.(this.actor, event.currentTarget.dataset.refitStart);
        if (!result?.ok) ui.notifications?.warn(`Could not start work: ${result?.reason ?? "unknown error"}.`);
        else { ui.notifications?.info(`Work started — ${result.job.remainingHours}h remaining.`); this.render({ force: true }); }
      } catch (error) { ui.notifications?.error(error.message); }
    });
  }
}
