...
function weaponFireControl(app, combatant, state, round) {
  ...
  for (const weaponState of weapons) {
    ...
    const workGunsAvailability = remaining > 0 && remaining <= 2
      ...
      : null;
    const canWorkGuns = Boolean(workGunsAvailability?.ok);
    ...
    row.innerHTML = `
      ...
      <div class="arkflight-gm-weapon-stats"><span>${damage.dice ?? "—"} ${titleCase(damage.type)}</span><span>Threat ${titleCase(weapon?.data?.systemThreat ?? "hull")}</span><span>1 AP fire</span><span>Reload ${weaponState.reloadRounds}</span><span>Range ${range.min ?? "—"} / ${range.optimalMin ?? "—"}–${range.optimalMax ?? "—"} / ${range.max ?? "—"}</span></div>
    ...
    row.querySelector("[data-reload-weapon]")?.addEventListener("click", async () => {
      try { await game.arkflight.combat.stationAction("common-reload-weapon", { weaponKey: weaponState.key }, combatant); app.render({ force: true }); }
    ...
    row.querySelector("[data-work-guns]")?.addEventListener("click", async () => {
      try { await game.arkflight.combat.stationAction("battlewatch-reload-weapon", { weaponKey: weaponState.key }, combatant); app.render({ force: true }); }
